...
STORMS_LIST_CSV = os.path.join(BASE_DIR, "scs_all_storms_1999_to_present.csv")
TEMP_DIR = os.path.join(BASE_DIR, "temp_yearly_data")
...
def fetch_multi_location_weather(start_date, end_date):
    """Tải dữ liệu thời tiết thô cho tất cả 8 trạm cùng một lúc."""
...
def fetch_multi_location_marine(start_date, end_date):
    """Tải dữ liệu sóng biển và hải lưu cho tất cả 8 trạm cùng một lúc."""
    lats = ",".join([str(STATIONS[name]["lat"]) for name in STATIONS])
    lons = ",".join([str(STATIONS[name]["lon"]) for name in STATIONS])
